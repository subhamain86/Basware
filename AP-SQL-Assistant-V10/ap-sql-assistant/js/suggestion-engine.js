(function (root) {
  ...
  var RULES = [
    ...
    { match: function (m) { return /Column "[^"]*" does not exist on table/.test(m); },
      suggest: 'Double-check the column name and the table it belongs to — use the "Pick Columns" search box, or browse Used Schema.' },
    ...
    { match: function (m) { return /A WHERE condition is required/.test(m); },
      suggest: 'Add at least one filter condition in the Filters card, or tick the explicit override checkbox if you are certain you want no WHERE condition.' },
